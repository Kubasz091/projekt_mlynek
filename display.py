...
from board import Board, Dot, Pawn
...
class Display:
    ...
    def type_item_into_display_list(self, item: list, coordinates: tuple):
        ...
        if (coordinates[0] + len(item[0])) > len(self._display_list[0]) or (
            coordinates[1] + len(item)
        ) > len(self._display_list):
    ...
    def create_full_display_list(self):
        ...
        self._headline_y = (
            int((board_height - (no_of_pawns_in_a_column * (pawn_height + 1))) / 2) + 2
        )
    ...
    def draw_display(self, wrapper):
        ...
        if (
            height >= len(baord_str) + 1
            and width >= len(baord_str[0])
            and self._game_lord._player1_won is False
            and self._game_lord._player2_won is False
            and self._game_lord._draw is False
        ):
            cursor_x, cursor_y = self._game_lord.keyboard_functionality(
                len(baord_str) + 1, len(baord_str[0])
            )
            ...
            statusbarstr = f"Press 'q' to exit | Press 'e' to move pawns | Pos: {int(cursor_x / 2)}, {cursor_y},   {player_str}"
